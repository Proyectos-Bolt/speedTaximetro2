...
function App() {
  ...
  const handlePositionUpdate = (position: GeolocationPosition) => {
    ...
    if (tripData.isRunning && !tripData.isPaused && lastPosition.current) {
      ...
      console.log('📏 Calculando distancia entre:');
      console.log('  Anterior:', {
        lat: lastPosition.current.latitude.toFixed(6),
        lng: lastPosition.current.longitude.toFixed(6)
      });
      console.log('  Actual:', {
        lat: newPosition.latitude.toFixed(6),
        lng: newPosition.longitude.toFixed(6)
      });
      console.log('  Distancia calculada:', distanceIncrement.toFixed(6), 'km');
      
      // Solo contar distancia si el movimiento es significativo (más de 1 metro)
      // y la precisión del GPS es razonable (menos de 20 metros de error)
      const minDistance = 0.001; // 1 metro en km
      const maxAccuracy = 20; // 20 metros de precisión máxima
      
      if (distanceIncrement > minDistance && position.coords.accuracy <= maxAccuracy) {
        console.log('✅ Movimiento válido detectado:');
        console.log('  Distancia:', distanceIncrement.toFixed(6), 'km');
        ...
        setTripData(prev => {
          ...
          console.log('  Distancia total:', newDistance.toFixed(3), 'km');
        ...
        // Actualizar la última posición solo si el movimiento fue válido
        lastPosition.current = newPosition;
      } else if (distanceIncrement <= minDistance) {
        console.log('⚠️ Movimiento muy pequeño, no se cuenta:', distanceIncrement.toFixed(6), 'km');
      } else {
        console.log('⚠️ Precisión GPS insuficiente:', position.coords.accuracy.toFixed(1), 'm (máx:', maxAccuracy, 'm)');
      }
    } else {
      // Si es la primera posición o el viaje no está activo, solo actualizar la referencia
      lastPosition.current = newPosition;
      console.log('📍 Posición de referencia actualizada');
    }
  };
